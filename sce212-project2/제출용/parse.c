...
instruction parsing_instr(const char *buffer, const int index)
{
	...
	if (hi_opcode == 0) {
		...
		instr.func_code= (num & 0b00000000000000000000000000111111) >> 0;
		fprintf(stderr, "\n\n hi_opcode : %d \n\n", instr.opcode); //이거 파싱 개잘됨. ok

		fprintf(stderr, "\n\n instr.r_t.r_i.rs : %d \n\n", instr.r_t.r_i.rs); //이거 파싱 개잘됨. ok
		fprintf(stderr, "\n\n instr.r_t.r_i.rt  : %d \n\n", instr.r_t.r_i.rt); //이거 파싱 개잘됨. ok
		fprintf(stderr, "\n\n instr.r_t.r_i.r_i.r.rd : %d \n\n", instr.r_t.r_i.r_i.r.rd); //이거 파싱 개잘됨. ok
		fprintf(stderr, "\n\n instr.r_t.r_i.r_i.r.shamt : %d \n\n", instr.r_t.r_i.r_i.r.shamt); //이거 파싱 개잘됨. ok
		fprintf(stderr, "\n\n instr.func_code: %d \n\n", instr.func_code); //이거 파싱 개잘됨. ok
	...
	//I 타입 ,S타입, J타입
		...
		if (hi_opcode == 0b000010 || hi_opcode == 0b000011) {
			...
			fprintf(stderr, "\n\n instr.r_t.r_i.rs : %d \n\n", instr.r_t.r_i.rs); //이거 파싱 개잘됨. ok
			fprintf(stderr, "\n\n instr.r_t.r_i.rt  : %d \n\n", instr.r_t.r_i.rt); //이거 파싱 개잘됨. ok
			fprintf(stderr, "\n\n instr.r_t.r_i.r_i.imm : %d \n\n", instr.r_t.r_i.r_i.imm); //이거 파싱 개잘됨. ok
